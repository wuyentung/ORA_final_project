...
def solve(df, DMU, NAME="temp"):
    ...
    for k in DMU:
        ...
        # 計算各process的效率值
        val_p1[k] = w_sol[0] * Z1[k] / ((v_sol[0]) * X1[k] + (v_sol[1]) * X2_1[k])
        val_p2[k] = w_sol[1] * Z2[k] / ((v_sol[1]) * X2_2[k] + (w_sol[0]) * Z1_1[k])
        val_p3[k] = (u_sol[0] * Y1[k] + u_sol[1] * Y2[k]) / ((w_sol[0]) * Z1_2[k] + (w_sol[1]) * Z2[k])
        
        # 計算各stage的效率值
        val_s1[k] = (w_sol[0] * Z1_2[k] + w_sol[1] * Z2[k]) / ((v_sol[0]) * X1[k] + (v_sol[1]) * X2[k])
        val_s2[k] = val_p3[k]
    ...
    for k in DMU:
        ...
        val_p2[k] = w_sol[2] * Z2[k] / (v_sol[1] * X2_2[k] + w_sol[0] * Z1_1[k])
        val_p3[k] = (u_sol[0] * Y1[k] + u_sol[1] * Y2[k]) / (w_sol[1] * Z1_2[k] + w_sol[2] * Z2[k])
        
        # 計算各stage的效率值
        val_s1[k] = (w_sol[1] * Z1_2[k] + w_sol[2] * Z2[k]) / (v_sol[0] * X1[k] + v_sol[1] * X2[k])
        val_s2[k] = val_p3[k]
    ...
    with pd.ExcelWriter('%s_sol.xlsx' %NAME) as writer:  
    ...
    with pd.ExcelWriter('%s_result.xlsx' %NAME) as writer:  
        data.to_excel(writer, sheet_name="data")
